...
#define vsnprintf _vsnprintf
  ...
  #define strtoll (long long)strtol
	...
	defined(__OpenBSD__))
/* currently incorrect */
static inline long double strtold(TCCState *st, const char *nptr, char **endptr)
{
    return (long double)strtod(nptr, endptr);
}
static inline float strtof(TCCState *st, const char *nptr, char **endptr)
{
...
static Sym *sym_find(TCCState *st, int v);
...
/* section generation */
static void section_realloc(TCCState *s,Section *sec, unsigned long new_size);
static void *section_ptr_add(TCCState *s,Section *sec, unsigned long size);
static void put_extern_sym(TCCState * s, Sym *sym, Section *section, 
                           unsigned long value, unsigned long size);
static void greloc(TCCState * st, Section *s, Sym *sym, unsigned long addr, int type);
static int put_elf_str(TCCState *st, Section *s, const char *sym);
static int put_elf_sym(TCCState *st, Section *s, 
                       unsigned long value, unsigned long size,
...
static void put_elf_reloc(TCCState *st, Section *symtab, Section *s, unsigned long offset,
                          int type, int symbol);
static void put_stabs(TCCState *st, const char *str, int type, int other, int desc, 
                      unsigned long value);
static void put_stabs_r(TCCState *st, const char *str, int type, int other, int desc, 
...
/* tcccoff.c */
int tcc_output_coff(TCCState *st, FILE *f);

/* tccpe.c */
static void *resolve_sym(TCCState *st, const char *sym, int type);
int pe_load_def_file(struct TCCState *st, Tcl_Channel fp);
void pe_setup_paths(struct TCCState *st, int *p_output_type, const char **p_outfile, char *first_file);
unsigned long pe_add_runtime(struct TCCState *st);
int tcc_output_pe(struct TCCState *st, const char *filename);
...
static void asm_expr(TCCState *st, ExprValue *pe);
static int asm_int_expr(TCCState *st);
static int find_constraint(TCCState *st, ASMOperand *operands, int nb_operands, 
                           const char *name, const char **pp);

static int tcc_assemble(TCCState *st, int do_preprocess);
...
static inline int is_float(TCCState *st, int t)
{
    int bt;
    bt = t & VT_BTYPE;
...
static inline int is_llong(TCCState *st, int t)
{
    int bt;
    bt = t & VT_BTYPE;
    return bt == VT_LLONG;
...
static void *resolve_sym(TCCState *st, const char *symbol, int type)
{
    TCCSyms *p;
    p = tcc_syms;
    while (p->str != NULL) {
...
static inline void *resolve_sym(TCCState *st, const char *sym, int type)
{
