...
static void asm_opcode(TCCState *st, int opcode)
{
    ...
    for(;;) {
        ...
        if (nb_ops >= MAX_OPERANDS) {
            error(st, "incorrect number of operands");
        }
        parse_operand(st, pop);
        if (tok == ':') {
           if (pop->type != OP_SEG || has_seg_prefix) {
               error(st, "incorrect prefix");
           }
           seg_prefix = *pop;
           has_seg_prefix = 1;
           next(st);
           parse_operand(st, pop);
           if (!(pop->type & OP_EA)) {
               error(st, "segment prefix must be followed by memory reference");
           }
        }
        pop++;
        nb_ops++;
    ...
    if (s == 3) {
        ...
        if (s == 3) {
            if ((opcode == TOK_ASM_push || opcode == TOK_ASM_pop) && 
                ...
                error(st, "cannot infer opcode suffix");
        }
    }
...
static inline int constraint_priority(TCCState *st, const char *str)
{
    ...
    for(;;) {
        ...
        case 'g':
            ...
            break;
        default:
            error(st, "unknown constraint '%c'", c);
            pr = 0;
        }
        if (pr > priority)
            priority = pr;
                                    ...
                                    int *pout_reg)
{
    ...
    for(i=0;i<nb_operands;i++) {
        ...
        if (isnum(st, *str) || *str == '[') {
            ...
            if ((unsigned)k >= i || i < nb_outputs)
                error(st, "invalid reference in constraint %d ('%s')",
                      i, str);
            op->ref_index = k;
            if (operands[k].input_index >= 0)
                error(st, "cannot reference twice the same operand");
    ...
    try_next:
        ...
        case '&':
            if (j >= nb_outputs)
                error(st, "'%c' modifier can only be applied to outputs", c);
            reg_mask = REG_IN_MASK | REG_OUT_MASK;
            goto try_next;
        case 'A':
        ...
        case 'g':
            ...
            break;
        default:
            error(st, "asm constraint %d ('%s') could not be satisfied", 
                  j, op->constraint);
            break;
        }
    ...
    *pout_reg = -1;
    for(i=0;i<nb_operands;i++) {
        ...
        if (op->reg >= 0 && 
            ...
            error(st, "could not find free output register for reloading");
        reg_found2:
            *pout_reg = reg;
            break;
        }
                              ...
                              SValue *sv, int modifier)
{
    ...
    if ((r & VT_VALMASK) == VT_CONST) {
        ...
        reg = r & VT_VALMASK;
        if (reg >= VT_CONST)
            error(st, "internal compiler error");
        ...
        if (modifier == 'b') {
            if (reg >= 4)
                error(st, "cannot use byte register");
            size = 1;
        } else if (modifier == 'h') {
            if (reg >= 4)
                error(st, "cannot use byte register");
...
static void asm_clobber(TCCState *st, uint8_t *clobber_regs, const char *str)
{
    ...
    if (reg >= TOK_ASM_eax && reg <= TOK_ASM_edi) {
        ...
        error(st, "invalid clobber register '%s'", str);
    }
    clobber_regs[reg] = 1;
}
