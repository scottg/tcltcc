...
static int64_t getclock_us(TCCState *st)
{
#ifdef WIN32
    struct _timeb tb;
    _ftime(&tb);
...
static int expand_args(TCCState *st, char ***pargv, const char *str)
{
    const char *st;
    char **argv, *arg;
    int argc, len;
    ...
    argv = NULL;
    for(;;) {
        while (is_space(st, *str))
            str++;
        if (*str == '\0')
            break;
        st = str;
        while (*str != '\0' && !is_space(st, *str))
            str++;
        len = str - st;
        arg = tcc_malloc(st, len + 1);
        memcpy(arg, st, len);
        arg[len] = '\0';
        dynarray_add(st, (void ***)&argv, &argc, arg);
    }
    *pargv = argv;
    return argc;
}
...
int parse_args(TCCState *s, int argc, char **argv)
{
    ...
    while (1) {
        ...
        if (r[0] != '-') {
            /* add a new file */
            dynarray_add(s, (void ***)&files, &nb_files, r);
            if (!multiple_files) {
                optind--;
        ...
        option_found:
            if (popt->flags & TCC_OPTION_HAS_ARG) {
                if (*r1 != '\0' || (popt->flags & TCC_OPTION_NOSEP)) {
                    ...
                    if (optind >= argc)
                        error(st, "argument to '%s' is missing", r);
                    optarg = argv[optind++];
                }
            } else {
                if (*r1 != '\0')
            ...
            case TCC_OPTION_I:
                if (tcc_add_include_path(s, optarg) < 0)
                    error(st, "too many include paths");
                break;
            case TCC_OPTION_D:
                {
                    char *sym, *value;
...
#endif
                        {
                            error(st, "target %s not found", p);
                        }
                    } else {
                        error(st, "unsupported linker option '%s'", optarg);
                    }
                }
                break;
            default:
                ...
                unsupported_option:
                    warning(st, "unsupported option '%s'", r);
                }
                break;
            }
        }
...
int main(int argc, char **argv)
{
    ...
    if (output_type == TCC_OUTPUT_OBJ && !reloc_output) {
        ...
        if (nb_objfiles != 1)
            error(st, "cannot specify multiple files with -c");
        if (nb_libraries != 0)
            error(st, "cannot specify libraries with -c");
    }
    ...
    if (output_type == TCC_OUTPUT_PREPROCESS) {
        if (!outfile) s->outfile = stdout;
            ...
            if (!s->outfile) error(st, "could not open '%s'", outfile);
        }
    } else if (output_type != TCC_OUTPUT_MEMORY) {
        if (!outfile) {
    /* compute default outfile name */
            pstrcpy(st,  objfilename, sizeof(objfilename) - 1, 
                    /* strip path */
        ...
        default_outfile:
            pstrcpy(st,  objfilename, sizeof(objfilename), "a.out");
        }
#endif
        outfile = objfilename;
        }
    ...
    for(i = 0;i < nb_files; i++) {
        ...
        if (output_type == TCC_OUTPUT_PREPROCESS) {
            ...
            if (tcc_add_library(s, filename + 2) < 0)
                error(st, "cannot find %s", filename);
